...
from dtale_desktop import actions
...
@router.get("/node/watch-profile-report-builder/{data_id}/")
async def node_watch_profile_report_builder(data_id: str):
    ...
    while time_waited < 600:
        if fs.profile_report_exists(data_id):
            return actions.UpdateNode(node=get_node_by_data_id(data_id))
        await asyncio.sleep(5)
        time_waited += 5
    return {
        "type": "notification",
        "payload": "The profile report failed to build for some reason",
    }
